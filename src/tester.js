 ...
 * @param {string} [testName] - test name (`undefined` when `eventName` is `finalize`)
 * @param {object} [assertion] - only defined when {eventName} is one of {["pass","fail","except"]}
 */
/*
...
const defaultReportHandler = (eventName, testName, assertion) => {
  ...
  switch (eventName) {
    ...
    case 'fail':
      if (reportVerbosity > 1) {
          ...
          if (assertion.mismatch.hasOwnProperty('values')) {
            ...
            console.log(
              `      - actual  : ${JSON.stringify(
                assertion.mismatch.values[0]
              )}`
            );
            console.log(
              `      - expected: ${JSON.stringify(
                assertion.mismatch.values[1]
              )}`
            );
          }
        ...
        if (assertion.hasOwnProperty('actualResult')) {
          console.log(`    * results`);
          console.log(
            `      - actual  : ${JSON.stringify(assertion.actualResult)}`
          );
          if (assertion.hasOwnProperty('expectedResult')) {
            console.log(
              `      - expected: ${JSON.stringify(assertion.expectedResult)}`
            );
          }
        } else if (assertion.hasOwnProperty('unexpectedResult')) {
          console.log(`    * results`);
          console.log(
            `      - unexpected: ${JSON.stringify(assertion.unexpectedResult)}`
          );
        }
      }
      break;
    case 'except':
      console.log(
        `  ${colors.except}exception${colors.reset}: ${assertion.msg}`
      );
      break;
    case 'finalize':
      if (reportVerbosity > 1) {
      ...
      console.log(
        `[tests] - ${
          counters.tests.passed > 0 ? colors.pass : colors.reset
        }passed${colors.reset}: ${counters.tests.passed}  ${
          counters.tests.failed > 0 ? colors.fail : colors.reset
        }failed${colors.reset}: ${counters.tests.failed}  ${
          counters.tests.skipped > 0 ? colors.skip : colors.reset
        }skipped${colors.reset}: ${counters.tests.skipped}`
      );
      console.log(
        `[assertions] - ${
          counters.assertions.passed > 0 ? colors.pass : colors.reset
        }passed${colors.reset}: ${counters.assertions.passed}  ${
          counters.assertions.failed > 0 ? colors.fail : colors.reset
        }failed${colors.reset}: ${counters.assertions.failed}  ${
          counters.assertions.exceptions > 0 ? colors.except : colors.reset
        }exceptions${colors.reset}: ${counters.assertions.exceptions}`
      );
  }
};
...
const tester = {
   ...
   * @param {number} [opt.repeat] - number of times test should be repeated (default = 1)
   */
  test: (testName, fn, opt) => {
     ...
     *
     * @param {boolean} skip - whether or not to skip this test
     */
    const _fn = (skip) => {
      if (skip) {
        /** @type {Promise<void>} */
    ...
    if (undefined !== opt.repeat) {
      const value =
        typeof opt.repeat === 'number' ? opt.repeat : parseInt(opt.repeat);
   ...
   * @param {string|string[]} [opt.tags] - only run tests with a given tag
   *
  ...
  setReportVerbosity: (level) => {
    const value = typeof level === 'number' ? level : parseInt(level);
    if (!isNaN(value)) {
      switch (value) {
        case 4:
        case 3:
